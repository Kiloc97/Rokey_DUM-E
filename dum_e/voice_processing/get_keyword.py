...
class GetKeyword(Node):
    def __init__(self):
        # appimage_path = os.path.join(package_path, 'resource/micclient-x86_64.AppImage')
        # # 권한 부여
        ...
        tigger_prompt_content ="""
            You have to determine whether User's statement mean <Yes> or <No>.
            I'm going to tip $200 for a better solution!
            Take a deep breath and work on this step by step.
            ### Instruction
            You will be penalized if you miss one of the instructions below
             - You must accurately extract the tasks included in the following OutputList from the sentence.
             - If the user's statement is judged to be <Yes>, you must return the value as <True>
             - If the user's statement is judged to be <No>, you must return the value as <False>
             - Returns <False> only when the user's statement clearly lacks identifiable intent or meaning.
            ### OutputFormat
             - You must follow following format : Trigger
             - Return only <True> or <False>    
            ### OutputList
             - ['True', 'False']
            ### Context
            Assume you are checking to the User about what the Users statement is correct.
            ### Example
    ...
    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함
        ...
        while True:
            ...
            self.get_logger().warn('명령 입력')
            output_message = self.stt.speech2text()
            self.get_logger().warn('이 명령이 맞나요?')
            trigger_message = self.stt.speech2text()
            # if trigger_message:
            #     self.trigger = SetBool(self.extract_keyword(self.trigger_lang_chain, trigger_message))
            #     self.get_logger().warn(f'{type(self.trigger)}')
            #     if self.trigger:
            #         break
            self.get_logger().warn(f'{trigger_message}')
            if trigger_message:
                trigger_result = self.extract_keyword(self.trigger_lang_chain, trigger_message).strip().lower()
                # trigger_result = self.extract_keyword(self.trigger_lang_chain, trigger_message).lower()
                self.get_logger().info(f"Trigger 판단 결과: {trigger_result}")

                if trigger_result == 'true':
                    break
                elif trigger_result == 'false':
                    self.get_logger().info("사용자가 음성 명령을 부정함. 다시 입력 대기.")
                else:
                    self.get_logger().warn(f"LLM 결과가 True/False가 아님: {trigger_result}")

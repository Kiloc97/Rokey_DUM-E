...
class ObjectDetectionNode(Node):
    def __init__(self, model_name = 'yolo'):
        ...
        #     self, ActionDetection, 'action_detection', self.action_detectoin_callback
        # )
        self._direction_server = self.create_service(SrvDepthPosition, 'service_detection', self.direction_detectoin_callback)
    ...
    def handle_model_change(self, request, response):
        try:
            if request.data == False:
                self.get_logger().info('model: best')
                self.model.switch_to_best_model()
            elif request.data == True:
                self.get_logger().info('model: over')
    ...
    def action_detectoin_callback(self, goal_hamdle):
        ...
        while True:
            ...
            if sum(coords) == 0:
                continue
            break
        self.get_logger().info('Action Server: Direction Detect')        # direction detection
        result = ActionDetection.Result()
    ...
    def direction_detectoin_callback(self, request, response):
        self.get_logger().info(f"Received request: {request.target}")     
        while True:
            coords = self._compute_position(request.target)
            if sum(coords) == 0:
                continue
            break
        self.get_logger().info('Service Server: Direction Detect')
        response.depth_position = [float(x) for x in coords]
        return response
